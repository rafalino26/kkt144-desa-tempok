...
// Hapus EditBadge, ganti dengan ikon Edit
...
interface HeroSectionProps {
  ...
  subtitle: string;
  isAdmin: boolean;
  onEditClick: () => void; // <-- Prop baru untuk membuka modal
}

export default function HeroSection({ title, subtitle, isAdmin, onEditClick }: HeroSectionProps) {
  return (
    // Styling section asli Anda
    <section className="relative rounded-2xl overflow-hidden
                       bg-gradient-to-br from-brand-primary/70 via-brand-light/60 to-brand-primary/70
                       border border-black/5 shadow-sm">

      {/* --- GANTI EditBadge MENJADI button --- */}
      {isAdmin && (
        <button
          onClick={onEditClick} // Panggil prop untuk modal
          // Styling mirip EditBadge, tapi sebagai button
